 ...
 */
 ...
 // My 
 // 로그는 정상적으로 다 찍히는데 테스트가 통과를 안한다... 노드 문제이길 
...
function phone(string, num) {
    ...
    const numReg = RegExp(num);
    let res = `Phone => ${num}`;
    
    let resArr = stringArr.filter(cur => cur.match(numReg));
    if(resArr.length > 1) return `Error => Too many people: ${num}`;
    if(resArr.length < 1) return `Error => Not found: ${num}`;

    for (let i = 0; i < stringArr.length; i++) {
      if(numReg.test(stringArr[i])) {
        console.log(stringArr[i])

        let name = /(<\w+ \w+>)|(<\w+>)/g.exec(stringArr[i])[0];
        name = name.substring(1, name.length - 1);

        const address = stringArr[i].replace(/<.+>|\+\d\d?-(\d{3}-){2}\d{4}|[;$*/?,:!]/g, "").replace(/_| +/g, " ");
